            ...
            <div className="flex flex-col sm:flex-row gap-2">
                <div className="grid grid-cols-2 sm:flex sm:flex-row gap-2 flex-1">
                    ...
                    <ServiceFilterSelect
                        ...
                        onChange={(val) => setFilters(prev => ({ ...prev, priorityLevel: val as Priority | "" }))}
                    />
                    <ServiceFilterSelect
                        label="Status"
            ...
            <div className="bg-gray-200/20 rounded-xl p-3 md:p-5 space-y-3">
                {isError ? (
                    <div className="p-6 text-center text-red-500 font-medium">
                        Unable to load customer services. Please check your connection and try again.
                    </div>
                    ...
                    <>
                        <h3 className="text-lg font-medium">
                            Customer Services:
                            <span className="text-gray-400"> {data?.content?.meta?.total ?? services.length}</span>
                        </h3>

                        <div className="space-y-2">
                            {services.map((service) => (
                                <CustomerServiceListCard key={service.id} service={service} />
                            ))}
                            {services.length === 0 && (
