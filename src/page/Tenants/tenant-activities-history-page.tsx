...
export default function TenantActivitiesHistoryPage() {
   ...
   const billingActivities = billings.flatMap(billing => billingToActivity(billing));

   const serviceActivities = services.map(service => serviceToActivity(service));
   ...
   const isLoading =
      isLoadingTenant || isLoadingBilling || isLoadingService || isLoadingContract;
   if (isLoading) return <LoadingSpinner label={"Loading tenant's activities..."} />;
                              ...
                              {!item.content.length && (
                                 <Card className="flex flex-col items-center justify-center h-[250px] text-center bg-content2/40 border border-default-200 shadow-none">
                                    ...
                                    <p className="text-sm text-foreground-400 mt-1">
                                       There are no {item.label.toLowerCase()}{" "}
