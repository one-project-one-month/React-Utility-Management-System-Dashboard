...
export function useConfirmDialog() {
    ...
    /** Open the confirmation dialog with merged defaults */
    const showConfirm = (option: ConfirmDialogOptions) => {
    ...
    /** Handle confirm and auto-close */
    const handleConfirm = async () => {
        if (!options?.onConfirm) return;
        setIsLoading(true);
        try {
            await options.onConfirm();
            closeDialog(); // ✅ Auto-close after async success
        } catch (error) {
                ...
                <ModalContent>
                    {() => (
                        <>
                            <ModalHeader className="flex flex-col items-center justify-center gap-2">
                                {options.icon}
                                <span className="text-2xl font-semibold">{options.title}</span>
                            </ModalHeader>
                            <ModalBody className="text-gray-600 text-center">
                                {options.message}
                            </ModalBody>
